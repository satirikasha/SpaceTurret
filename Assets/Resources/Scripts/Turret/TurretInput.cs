      ...
      if(overallDeltaPos.sqrMagnitude > (Screen.width / Settings.MinDeltaPosition).deg2()) {
        if(Mathf.Abs(overallDeltaPos.x) >= Mathf.Abs(overallDeltaPos.y)) {
          if(!BlockHorizontalInput) {
            var predictedAngle = Turret.TargetAngle - Settings.AngleSensitivity * eventData.delta.x / Screen.width;
          ...
          if(!BlockVerticalInput){
            var predictedForce = Turret.TargetForce + Settings.ForceSensitivity * eventData.delta.y / Screen.height;
