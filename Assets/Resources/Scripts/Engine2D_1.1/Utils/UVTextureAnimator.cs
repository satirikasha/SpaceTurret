  ...
  internal class UVTextureAnimator: MonoBehaviour {
    ...
    private void InitDefaultVariables() {
      ...
      if(AnimatedMaterialsNotInstance.Length > 0)
        ...
        if(IsBump) {
          _Renderer.material.SetTextureScale("_BumpMap", size);
          _Renderer.material.SetTextureOffset("_BumpMap", offset);
        }
        if(IsHeight) {
          _Renderer.material.SetTextureScale("_HeightMap", size);
          _Renderer.material.SetTextureOffset("_HeightMap", offset);
        }
      }
    }
